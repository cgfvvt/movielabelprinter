...
Implementation took around 15 hours total (from reading requirements till code deployed and notification posted)
...
Download the movie from: "https://tup-ops-eng-coding-exercise.s3.amazonaws.com/data/{0}.json" replace {0} with the movie id.
Print the label
...
2.1.  The application is intended for use by single person occasionally printing labels to stick them on DVD boxes or other packages
...
2.5.  No assumptions can be made about data types returned in JSONs from web server; thus they all considered of type string
2.6.  Testing should be done only on range of movie IDs 1-6, as only those seem to be available
2.7.  In case if other movie IDs will produce extra properties in JSON, they should be silently ignored
2.8.  Font family is hard-coded in application, and it is "Serif" family; this approach seem to work reliably on Intermec and Zebra printers
2.9.  Font size absolute values should be limited by range from 4 to 144
2.10. Application should use default printer setting for specified printer, including paper size and orientation
...
Only manual testing was performed due to large portion of environment sensitive code and limited time.

4. Usage
--------

To print movie labels: start application, enter movie Id and select printer from list, then click "Print" button.
